...
def write_sheet(ws, merge_map, n_header_rows):
    # Paso 1: aplicar TODOS los merges primero, antes de tocar ninguna celda.
    # openpyxl rompe un merge si se escribe en una celda que ya pertenece a él.
        ...
        for cell_info in row_data:
            ...
            if cs > 1 or rs > 1:
                ...
                ws.merge_cells(
                    ...
                    end_row=r_idx + rs - 1,  end_column=xl_col + cs - 1,
                )

    # Paso 2: escribir contenido y formato en la celda ancla de cada merge.
    for r_idx, row_data in enumerate(merge_map, start=1):
        for cell_info in row_data:
            xl_col  = cell_info["xl_col"]
            ...
            xl_cell.alignment = Alignment(
                ...
                horizontal = "center" if (cell_info["col_span"] > 1
                                          or cell_info["row_span"] > 1) else "left",
            )
            xl_cell.border = CELL_BORDER
            if fill_hex:
                xl_cell.fill = PatternFill("solid", start_color=hex_to_aRGB(fill_hex))
